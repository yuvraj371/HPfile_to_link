...
async def login_handler(c: Client, m: Message):
    try:
        try:
            ag = await m.reply_text("Now send me password.\n\n for password send 10rs to `hp148981@paytm` and send screen shot to @hpsupportbot \n\n (for running bot need contributions..unwanted message = ban + report)")
...
async def private_receive_handler(c: Client, m: Message):
    if MY_PASS:
        ...
        if check_pass== None:
            await m.reply_text("ʟᴏɢɪɴ ғɪʀsᴛ ᴜsɪɴɢ /login ᴄᴍᴅ(ᴛʏᴘᴇ /login)  \n\n for password send 10rs to `krishna527062@oksbi` and send screen shot to @Madhuri_niranjan \n\n (for running bot need contributions..unwanted message = ban + report)")
            return
...
async def channel_receive_handler(bot, broadcast):
    if MY_PASS:
        ...
        if check_pass == None:
            await broadcast.reply_text("Login first using /login cmd \n don\'t know the pass? request it from @Madhuri_niranjan")
            return
        if check_pass != MY_PASS:
